...
const AddAdherentModal = ({ show, onHide, onAdd }) => {
  ...
  // Fonction de validation du téléphone avec une regex
  ...
  const handleChange = (e) => {
    ...
    setErrors((prevErrors) => ({ ...prevErrors, [name]: '' })); // Nettoyer l'erreur sur modification
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onAdd(formData); // Appel à la fonction du parent (dispatch + unwrap à faire côté parent)
      setFormData({ nom: '', prenom: '', email: '', telephone: '', actif: true });
      onHide(); // Fermer la modale uniquement si succès
    } catch (err) {
